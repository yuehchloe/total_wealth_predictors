...
# setwd("C:/Users/yuehc/Google Drive/Archive/College/10-WI21/ECON178")
...
# ira, nifa, inc, educ might need spline transformations
...
cbind(0:(max_knots - 2), MSPE)
# For inc, 0 knot has the lowest MSPE, no need for transformation
...
MSPE <- colMeans(MSPE, na.rm = TRUE)
cbind(0:(max_knots - 2), MSPE)
# For educ, 5 knots has the lowest MSPE
...
# Results say that Stepwise backward is the best out of the 4 models

# Closer look at Stepwise Backward
...
# From stepwise selection, we see that ira, e401, nifa, inc, hmort, hval, male, twoearn, age are significant

# So we include use these variables to create a GAM Model
gam <- lm(tw ~ ns(ira, 11) + e401 + ns(nifa, 11) + inc + hmort + hval + male + twoearn + ns(age, 3), data=data_in)
mse_gam <- mean(residuals(gam)^2)
summary(gam)

c(mspe_step_backward, mspe_step_forward, mspe.lasso, mspe.ridge, mse_gam)

# Summary of GAM model shows that certain knots are not statistically significant
# Create new variables for the significant knots of ira
data_in$ira_ns8 <- ns(data_in$ira, 11)[,8]
data_in$ira_ns9 <- ns(data_in$ira, 11)[,9]
data_in$ira_ns10 <- ns(data_in$ira, 11)[,10]
data_in$ira_ns11 <- ns(data_in$ira, 11)[,11]

# Create new variables for the significant knots of nifa
data_in$nifa_ns7 <- ns(data_in$nifa, 11)[,7]
data_in$nifa_ns8 <- ns(data_in$nifa, 11)[,8]
data_in$nifa_ns9 <- ns(data_in$nifa, 11)[,9]
data_in$nifa_ns10 <- ns(data_in$nifa, 11)[,10]
data_in$nifa_ns11 <- ns(data_in$nifa, 11)[,11]

# Create new variables for the significant knots of age
data_in$age_ns3 <- ns(data_in$age, 3)[,3]

# Fit the new model using only the significant knots
new_model <- lm(tw ~ ira_ns8 + ira_ns9 + ira_ns10 + ira_ns11 + e401 + nifa_ns7 + nifa_ns8 + nifa_ns9 + nifa_ns10 + nifa_ns11 + inc + hmort + hval + twoearn + age_ns3, data = data_in)
